...
- Контейнер стартует командой `npm run start`, приложение слушает `0.0.0.0:3000`.
- Healthcheck проверяет `GET /api/health` на `127.0.0.1:3000`.
- Проект полностью статический по данным: БД и Prisma не используются.
