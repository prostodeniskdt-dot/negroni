...
- Контейнер всегда стартует на `0.0.0.0:3000` (healthcheck ходит в `GET /api/health`).
- Переменная `PORT` в рантайме игнорируется намеренно, чтобы деплой-платформа стабильно получала ответ на порту `3000`.
- Инициализация БД при старте контейнера по умолчанию выключена (`STARTUP_DB_INIT=0`), чтобы не блокировать readiness.
- Если вам нужно выполнять `prisma migrate deploy` при старте, включите `STARTUP_DB_INIT=1`.
