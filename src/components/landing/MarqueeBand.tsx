    ...
    >
      ...
      >
        {[...items, ...items].map((item, i) => (
          <span
            key={`marquee-${i}`}
            className="flex items-center gap-6 mx-6 font-display text-sm md:text-base tracking-[var(--letter-spacing-wide)] uppercase text-[var(--color-text-muted)]/85 font-medium leading-normal hover:text-[var(--color-accent)] transition-colors duration-500 cursor-default"
          >
            {item}
